...
3. 当一个无样式的空span元素被赋予以下属性时，其display属性的计算值
   ...
   **如果一个元素是绝对定位元素(display为fixed或absolute)，float的值设置为none，对于浮动元素或绝对定位元素，计算值由声明值确定；对于根元素，如果声明值为(inline-)table，都会得到计算值table，生命为none时会得到计算值none，其他所有display值都计算为block **
